...
func _physics_process(_delta: float) -> void:
	...
	if frame == 60:
		...
		# Walking up the bridge really is uphill -- what makes a loose plinko
		# ball roll back down at the players.
		check(player.position.y > start_position.y + 0.3,
			"and gains height doing it (%.2f m)" % (player.position.y - start_position.y))
		finish()
